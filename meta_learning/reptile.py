...
def meta_update_step(
    ...
    with torch.no_grad():
        for name, p in model.named_parameters():
            ...
            adapted_param_values = [ap[name] for ap in adapted_params_list]
            avg_adapted_param = torch.stack(adapted_param_values, dim=0).mean(0)

            # Reptile update: interpolate between original and average adapted
            p.add_(meta_lr * (avg_adapted_param - p))
