...
from meta_learning.reptile import adapt_model, meta_update_step, clone_model
...
inner_lr = 1e-2
meta_lr = 1e-3  # Reptile step size
inner_steps = 5
meta_batch_size = 16
num_epochs = 1000

# Note: meta_optimizer is not used in Reptile, but we pass it for consistency
meta_optimizer = None
...
for epoch in range(num_epochs):
    ...
    # Meta update step (Reptile parameter averaging)
...
plt.title('Reptile Training - Meta Loss')
plt.xlabel('Epoch')
plt.ylabel('Meta Loss')
plt.yscale('log')
plt.grid(True)
plt.savefig('/workspaces/meta-learning/reptile_training_curve.png')
plt.show()

print("Reptile training completed!")
